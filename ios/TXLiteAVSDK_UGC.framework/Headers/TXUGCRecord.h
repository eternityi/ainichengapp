 ...
 *      -5 licence 验证失败，您可以通过 getLicenceInfo 接口查询licence信息，
                   检查licence的bundleid 是否和工程一致，licence是否还在有效期之内
...
/**
 * setMotionMute  设置动效静音 （增值版本有效，普通版本设置此参数无效）
 * 参数：
 ...
 * return:
 *        音乐时长 s (返回值为0，表示BGM格式不支持或则音频解析失败)
 */
 ...
 * 截图/拍照,startCamera 之后调用有效
 @params snapshotCompletionBlock 完成回调
 */
